...
| HW-408 | Verify X News is absent while Off and when no xAI API key is configured. Save a key in the Mac companion, enable X News through its confirmation, refresh once, and compare the bounded host cache with the inserted page; vertically scroll up to fifteen posts, open several detail views, use Back, then swipe horizontally to Weather and back. With both an empty and populated cache, pull below/above the board threshold; repeat from either surface during cooldown/in-flight, then disable and re-enable around an existing cache. | Off/unavailable hides the complete page; enabled inserts it in the right order. The Mac shows Keychain configuration, current cache count/age, exact last-request cost, and fetching/result/cooldown state; Mac and board refresh actions share one in-flight job and cooldown. The empty-state pull gives immediate pull/release feedback, an animated center-card loading state, and a terminal result held for at least eight seconds. At most fifteen bounded posts expose legible category/confidence chips; each row opens the matching author, timestamp, summary, available post text, and direct URL. Vertical and horizontal gestures do not conflict. Cached data never bypasses Off, and no raw output or credentials appear. | Redacted cache fields + companion/board photos + gesture video + host/serial log | PARTIAL 2026-08-12: physical empty-state pull queued and sent promptly; spinner was visually confirmed. One historical uninterrupted authenticated Grok CLI run produced five newly cited stories that passed the strict local validator, were cached, securely synced over TLS-PSK, and appeared on the physical 5B. Refined typography firmware was hash-verified, flashed, booted cleanly, restored Wi-Fi, and reconnected securely. The new Responses API path, populated-feed vertical scrolling, and final on-panel typography judgment remain pending physical validation. |
...
| HW-411 | USB-flash a signed bridge, confirm it boots as the initial valid slot and Settings/Mac show signed OTA ready, publish a newer signed candidate, request install from each UI, reboot, and observe at least 30 seconds. Repeat the newer candidate with an intentional pre-confirmation reset. | The bridge boots without a loop and restores NVS; download progress reaches verification; a healthy later OTA candidate becomes valid only after the stability window; a failed later candidate automatically returns to the previous UI/settings. | Redacted serial log + before/after photos + slot states | PARTIAL 2026-08-14: a verified 16 MiB recovery backup exists. Signed v0.2.10 was USB-flashed with NVS preserved and every written region hash-verified. The physical board booted stably from `ota_0`, restored Wi-Fi/pairing, synchronized network time asynchronously, refreshed live weather, and started the 6144-byte static OTA worker without a startup allocation race. Manual Settings checks completed HTTPS certificate validation with 2,132 bytes of OTA stack reserve. Signed v0.2.11 sequence 9 and v0.2.13 sequence 11 each downloaded, verified, booted from the alternate slot, and safely rolled back to v0.2.10 during confirmation; OTA metadata explicitly showed v0.2.11 as `INVALID`. The v0.2.13 hands-off timing exposed an invalid 32 KiB free-internal-heap verdict for this PSRAM/display design. After removing that false proxy, signed v0.2.14 sequence 12 was published image-first/manifest-last, verified byte-for-byte, installed from Settings over mobile Wi-Fi, booted with its new version visible, remained stable beyond the 30-second confirmation window, and did not roll back. On 2026-08-14, v0.2.17 reproduced a pre-display startup-stack corruption and was recorded `ABORTED`; v0.2.18 sequence 16 then proved the stack repair but exhausted LVGL's fixed 128 KiB pool and was deliberately reset before confirmation, returning to v0.2.16. A first system-heap allocator attempt in v0.2.19 rendered the UI but starved Wi-Fi RX buffers. Signed v0.2.20 sequence 18 instead pinned a 256 KiB LVGL pool directly to PSRAM; its USB recovery flash preserved NVS, booted the complete UI, allocated all 10 Wi-Fi RX buffers, rejoined the remembered mobile hotspot, obtained an IP, synchronized time, refreshed live weather, and reconnected to the paired Mac over Wi-Fi. Its public image was downloaded and compared byte-for-byte before the sequence-18 manifest was published. Confirmation reuses the reserved updater worker; active update states suppress screensaver/display-off and expose starting/first-boot verification progress. Companion-originated installation remains pending. |
...
Framebuffer screenshot transport is implemented and hardware-independently verified, but HW-407 remains its physical fidelity/reliability release gate; panel photos are still authoritative until it passes. The physical board now runs signed v0.2.20 after a verified NVS-preserving USB recovery flash; it renders the complete UI, initializes all Wi-Fi buffers, reconnects to the remembered hotspot and paired Mac, and refreshes live weather. Earlier v0.2.11, v0.2.13, v0.2.17, and v0.2.18 candidates demonstrated safe automatic rollback or deliberate pre-confirmation recovery. Production confidence remains gated on completing HW-411's companion-originated install, HW-412 fault injection, durable key custody, and recovery evidence; the verified full-flash backup remains the recovery path for this first device.
