...
| HW-102 | With X News Off, swipe Dashboard → Codex → Weather → Settings and back ten times. Enable X News on the Mac, wait for sync, repeat through Dashboard → Codex → X News → Weather → Settings, then disable it while X News is open. | Navigation expands from four to five pages and contracts back without a gap, wrap, stuck tile, inverted gesture, or hidden selected page; disabling while open lands safely on Weather. | Video + cycle count | NOT RUN |
...
| HW-307 | Cold-boot ten times while recording the panel and serial output; tap a visible Dashboard target during the Pulse ignition on one run. | No uninitialized or torn frame appears before the first Dashboard frame; the signal rail/roundel effect completes smoothly in under 500 ms; the serial log reports first-frame time; touch finishes the effect without delaying or freezing startup. | Slow-motion video + ten first-frame timings + touch video | NOT RUN |
...
| HW-408 | Verify X News is absent while Off and when Grok is unavailable. Enable it through the Mac confirmation, refresh once, and compare the verified host cache with the inserted page; vertically scroll five stories, then swipe horizontally to Weather and back. Pull below/above the threshold, repeat during cooldown/in-flight, then disable and re-enable around an existing cache. | Off/unavailable hides the complete page; enabled inserts it in the right order. At most five bounded stories match category/confidence/source handle; vertical and horizontal gestures do not conflict. Refresh states are honest, only one Grok process starts, cached data never bypasses Off, and no raw output or credentials appear. | Redacted cache fields + page photos + gesture video + host log | NOT RUN |
...
| HW-411 | With a known-good image in the inactive slot, install a correctly signed candidate, reboot, and observe at least 30 seconds. Repeat with an intentional pre-confirmation crash/reset. | Healthy candidate becomes valid only after the stability window; failed candidate automatically returns to the previous UI/settings without a boot loop. | Redacted serial log + before/after photos + slot states | NOT RUN |
| HW-412 | Attempt an unsigned image, an image signed by an untrusted key, a corrupt/truncated image, and a power cut during write. | Every invalid/incomplete artifact is rejected; the active slot and NVS remain usable; USB recovery remains possible. | Artifact hashes + redacted verifier/serial logs | NOT RUN |
...
Framebuffer screenshot transport is implemented and hardware-independently verified, but HW-407 remains its physical fidelity/reliability release gate; panel photos are still authoritative until it passes. The dual-slot rollback and signed-artifact policy foundation is compile-verified, but delivery remains disabled until HW-411/HW-412, production key custody, hosting, and recovery procedures pass. USB flash remains the supported firmware update path.
