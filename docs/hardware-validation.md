...
Framebuffer screenshot transport is implemented and hardware-independently verified, but HW-407 remains its physical fidelity/reliability release gate; panel photos are still authoritative until it passes. The physical board now runs signed v0.2.14, installed wirelessly from the signed v0.2.10 bridge and accepted after its 30-second first-boot stability window. Earlier v0.2.11 and v0.2.13 candidates demonstrated safe automatic rollback. Production confidence remains gated on completing HW-411's companion-originated install and deliberate reset exercise, HW-412 fault injection, durable key custody, and recovery evidence; the verified full-flash backup remains the recovery path for this first device.
