...
| HW-408 | With X News explicitly enabled on the Mac, refresh once and compare the verified host cache with the fifth page; vertically scroll a five-story feed, then swipe horizontally to Weather and back. Pull down below/above the threshold, repeat during cooldown/in-flight, and test with consent disabled and Mac offline; finally age/fail the cache. | At most five bounded stories appear in the right order with matching category/confidence/source handle; vertical drags reveal stories four/five, horizontal drags still change pages, and neither gesture causes an accidental page/scroll jump. Sub-threshold pull is inert; accepted pull shows fetching then result; disabled/cooldown/busy/offline states are honest; only one Grok process starts; no raw output or credentials appear; rejected/stale data never replaces the last verified feed. | Redacted cache fields + page photos + gesture video + host log | NOT RUN |
