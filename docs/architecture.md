...
- Optional X News is a Mac-side adapter, not direct ESP32 Internet access. The companion detects the Grok executable and owns explicit consent plus schedule state; its snapshot boolean inserts or removes the complete LVGL page. An enabled scheduler or authenticated pull-to-refresh request runs headless `grok -p` for one focused X search, structurally validates direct X URLs and their ID-derived timestamps, and caches up to fifteen bounded posts without a separate claim-verification pass. The optional LVGL page never executes Grok or holds X credentials; its request cannot enable consent or bypass cooldown/validation. Each row opens a local read-only detail overlay from the already authenticated snapshot.
