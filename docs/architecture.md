...
- Optional X News is a Mac-side adapter, not direct ESP32 Internet access. The companion stores an explicitly entered xAI API key in the device-only, non-synchronizing login Keychain and owns paid-API consent plus schedule state; its snapshot boolean inserts or removes the complete LVGL page. An enabled scheduler or authenticated pull-to-refresh request calls the xAI Responses API with required X Search, `store: false`, a two-turn tool limit, and a strict bounded schema, then independently validates direct X URLs and their ID-derived timestamps before caching up to fifteen posts. The optional LVGL page never calls xAI or holds the key, cost, prompt, tool output, or raw response; its request cannot enable consent or bypass cooldown/validation. Each row opens a local read-only detail overlay from the already authenticated snapshot.
