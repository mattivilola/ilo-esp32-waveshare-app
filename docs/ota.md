...
5. For the one-time bridge, run `make firmware-release-flash PORT=/dev/cu...`. It needs only the public verification key, re-verifies the signed release, and writes bootloader, partition table, initial OTA metadata, and the signed app while deliberately preserving NVS at `0x9000` (Wi-Fi, pairing, weather, and device settings). The bridge starts as the initial valid slot; subsequent wireless candidates enter `PENDING_VERIFY`. Flash never builds or signs.
