...
Results are capped at ten most-recent tasks and cached for 15 seconds. The companion owns one App Server child for its lifetime so a confirmed fixed action can resume a selected task; it never reads or writes Codex SQLite databases, session logs, or internal state files directly.
...
Optional X News remains Mac-mediated and disabled by default. The companion accepts the xAI API key only through a masked field and stores it as a generic password with `AfterFirstUnlockThisDeviceOnly` accessibility and Keychain synchronization disabled. The saved value can be replaced or removed but is never read back into the UI, preferences, logs, process arguments, cache files, diagnostic state, or board payloads. It exists in memory only while constructing the HTTPS bearer request to `api.x.ai`. Enabling X News requires explicit CLI consent or the companion’s confirmation dialog because API usage consumes separately billed xAI credits. The consent version was advanced for this billing change, so an earlier Grok CLI schedule cannot silently authorize API requests. A missing key, stale consent, or Off schedule sends a false visibility flag and hides the complete board page; the ESP32 cannot enable it.

The adapter calls the xAI Responses API with required X Search, `store: false`, a two-turn tool limit, a three-minute deadline, and a 1 MB response cap. It requires evidence that the search tool ran, decodes a strict bounded schema, and then rejects stories without matching direct X status URLs whose Snowflake timestamps fall inside the requested 24-hour window. It does one focused search without an independent claim-verification pass. Invalid output never replaces the previous cache. The exact API-reported request cost may be retained locally for the companion status, but only the visibility flag and bounded feed—headline, summary, available post text, category, confidence, author, timestamp, and direct URL—cross TLS to the board; the API key, cost, reasoning, prompts, usage, tool calls, and raw response stay off the wire.

Pull-to-refresh is a narrow authenticated request, not a general command channel. It is accepted only after TLS hello/subscription, carries a random bounded request ID and no prompt, and cannot enable paid-API consent. The Mac independently enforces opt-in, a 15-minute cost cooldown, one API request at a time, the same timeout/validator, and last-good-cache preservation. The board receives only one of six coarse result states and never sees raw API errors.

Codex control is a narrow authenticated request, not a general prompt channel. It is off by default and requires a fresh version-2 opt-in in the Mac companion, where it can be revoked immediately. The board sends only a random one-time request ID, a currently displayed task ID, and `continue`, `approvePlan`, or `rejectPlan`. The Mac rechecks consent, rejects malformed or recently replayed IDs, re-lists the ten current tasks, and re-fetches the newest turn. Continue is allowed only for idle/unloaded tasks and constructs exactly `Please continue.` locally. Plan responses additionally require a newest completed plan and its model: approval starts default mode with the re-fetched plan, while rejection starts Plan mode with a fixed revision request. The board sends neither prompt nor plan text. Active tasks, stale plans, failures, unknown IDs, command approvals, arbitrary text, and permission changes fail closed.
