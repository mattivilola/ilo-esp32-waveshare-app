...
3. Host replies `helloAck` with `tasks.read`, bounded `tasks.chat.read`, fixed `tasks.continue.fixed` and `tasks.plan.fixed`, `macPower.read`, and the gated `xNews.refresh.request` capability. Dashboard snapshots add `xNews.read` only when a bounded X News cache is present.
4. Board sends `subscribe` and advertises `tasks.read`, bounded `tasks.chat.read`, fixed `tasks.continue.fixed` and `tasks.plan.fixed`, diagnostic `display.capture.rgb565`, and gated `xNews.refresh.request` support in `hello`.
...
An authenticated board may send `xNewsRefreshRequest` after hello/subscription when the user pulls down at the top of X News. The host replies with bounded `xNewsRefreshStatus` states: `fetching`, `updated`, `disabled`, `cooldown`, `busy`, or `failed`. This never bypasses Mac-side paid-API opt-in, API-key availability, the 15-minute cost cooldown, one-in-flight limit, direct-X/timestamp validation, or cache preservation. See `x-news-refresh-v1.schema.json`.

An authenticated board may send `codexChatRequest` only after the user selects one of the ten tasks in the latest visible snapshot. The companion reads at most four recent turns, keeps only the newest six user/assistant text messages, sanitizes each to at most 360 board-safe characters, and returns them with coarse state, attention, update time, and at most two available fixed actions in `codexChatDetail`. Commands, tool output, reasoning, file paths, approval payloads, attachments, and hidden metadata never cross to the board. The board keeps selection separate from opening the read-only, vertically scrollable conversation and shows the bounded metadata alongside it; its privacy setting can suppress the request entirely. See `codex-chat-v1.schema.json`.

After explicit Mac-side opt-in, an authenticated board may send `codexContinueRequest` only after the user selects a recent task, holds one offered action to arm it, and taps a separate confirmation. The request carries a one-time bounded ID, one of the ten currently visible task IDs, and one fixed token: `continue`, `approvePlan`, or `rejectPlan`; it never carries prompt or plan text. The companion rechecks consent, current task state, replay ID, and the newest completed turn. Continue constructs exactly `Please continue.`. Plan approval re-fetches the latest completed plan and starts a default-mode turn containing `PLEASE IMPLEMENT THIS PLAN:` plus that plan. Rejection starts a Plan-mode turn with the fixed request `Please revise the proposed plan before implementation.` Command approvals, active-turn steering, arbitrary text, permission changes, and stale plans remain unsupported. See `codex-continue-v1.schema.json`.
