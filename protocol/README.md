...
3. Host replies `helloAck` with `tasks.read`, fixed `tasks.continue.fixed`, `macPower.read`, and the gated `xNews.refresh.request` capability. Dashboard snapshots add `xNews.read` only when a verified cache is present.
4. Board sends `subscribe` and advertises `tasks.read`, fixed `tasks.continue.fixed`, diagnostic `display.capture.rgb565`, and gated `xNews.refresh.request` support in `hello`.
